        ...
        <div class="col-lg-6">
            ...
            <div class="card card-modern p-4 mb-4">
                <form action="{{ route('cek-status.cek') }}" method="POST" class="d-flex gap-2">
                    @csrf
                    <input type="text" name="kode_transaksi" value="{{ $kode ?? old('kode_transaksi') }}" class="form-control" placeholder="Contoh: DON-000001">
            ...
            @if ($sudahDicari ?? false)
             @if ($data)
                <div class="card card-modern p-4">
                    ...
                    <div class="row mb-2"><div class="col-5 text-muted">Jenis</div><div class="col-7">{{ $tipe === 'donasi' ? ($data->jenisDonasi->nama ?? $data->nama_jenis_donasi_snapshot ?? '-') : ($data->paketKurban->nama_paket ?? $data->nama_paket_snapshot ?? '-') }}</div></div>
