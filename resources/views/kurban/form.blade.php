...
<div class="container py-5">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            ...
            <p class="section-subtitle text-center">Pilih jenis hewan kurban sesuai kemampuan Anda</p>

            @if ($errors->any())
                <div class="alert alert-danger">
                    <strong>Terjadi kesalahan, mohon periksa kembali form Anda:</strong>
            ...
            <form action="{{ route('kurban.store') }}" method="POST" id="formKurban">
                ...
                <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            <div class="card card-modern p-3 h-100 paket-option" data-id="{{ $paket->id }}" data-hewan="sapi" style="cursor:pointer;">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="paket_kurban_id" value="{{ $paket->id }}" id="paket{{ $paket->id }}" data-hewan="sapi" {{ old('paket_kurban_id') == $paket->id ? 'checked' : '' }}>
                    ...
                    @foreach ($paketKambing as $paket)
                        <div class="col-md-6">
                            <div class="card card-modern p-3 h-100 paket-option" data-id="{{ $paket->id }}" data-hewan="kambing" style="cursor:pointer;">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="paket_kurban_id" value="{{ $paket->id }}" id="paket{{ $paket->id }}" data-hewan="kambing" {{ old('paket_kurban_id') == $paket->id ? 'checked' : '' }}>
                                    <label class="form-check-label fw-semibold" for="paket{{ $paket->id }}">🐐 {{ $paket->nama_paket }}</label>
                                </div>
                                <p class="text-muted small mt-2 mb-1">{{ $paket->deskripsi }}</p>
                    ...
                    @endforeach
                    @error('paket_kurban_id')
                        <div class="col-12"><div class="text-danger small">{{ $message }}</div></div>
                    @enderror
                </div>

                <div id="slotContainer" class="card card-modern p-4 mb-4 {{ old('paket_kurban_id') && $paketSapi->contains('id', (int) old('paket_kurban_id')) ? '' : 'd-none' }}">
                ...
                <div class="card card-modern p-4">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" inputmode="numeric" name="whatsapp" value="{{ old('whatsapp') }}" class="form-control @error('whatsapp') is-invalid @enderror" placeholder="08xxxxxxxxxx" pattern="[0-9]*" maxlength="15" onkeypress="return event.charCode >= 48 && event.charCode <= 57" oninput="this.value = this.value.replace(/\D/g, '')">
                            @error('whatsapp')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
...
<script>
    ...
    function loadSlot(paketId) {
        fetch(`/kurban/slot/${paketId}`)
            .then(res => res.json())
            .then(data => {
                slotList.innerHTML = '';
                data.forEach(slot => {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const checkedSapi = document.querySelector('input[name="paket_kurban_id"][data-hewan="sapi"]:checked');
        if (checkedSapi) {
            checkedSapi.closest('.paket-option').classList.add('border', 'border-primary');
            loadSlot(checkedSapi.value);
        }
    });
</script>
@endpush